    ...
    <div :class="[ openModal ? 'is-active' : '', 'modal' ]">
      ...
      <div class="modal-card font17">
        <header class="modal-card-head">
          <p v-if="!isUserSignedUp" class="modal-card-title font17">{{ modalTitle }}</p>
          <p v-if="isUserSignedUp" class="modal-card-title font17">{{ modalTitleRegistered }}</p>
        ...
        <form @submit="checkForm" action="#" method="post">
          <section class="modal-card-body">
            <div v-if="!isUserSignedUp">
              <div class="field">
                <p class="control has-icons-left has-icons-right font17">
                  <input
            ...
            <div v-if="isUserSignedUp" class="level">
              <div class="level-item has-text-centered">
                <div>
                  <p class="title">عزیز خوش آمدی {{ name }}</p>
                  <p class="heading font17 has-text-success">هم اکنون شما یه عضو جدید هستید</p>
                </div>
              </div>
            </div>
          </section>
          <footer class="modal-card-foot">
            <button v-if="!isUserSignedUp" class="button is-success font17">{{ primaryBtnLabel }}</button>
            <button v-if="isUserSignedUp" type="button" class="button is-info font17" @click="closeModal">{{ btnRegisteredLabel }}</button>
          </footer>
        </form>
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      modalTitle: 'ثبت نام',
      modalTitleRegistered: 'خوش آمدید ',
      primaryBtnLabel: 'ثبت نام',
      btnRegisteredLabel: 'بستن',
      namePlaceholder: 'نام*',
      emailPlaceholder: 'ایمیل*',
      passwordPlaceholder: 'رمز عبور*',
      repeatPasswordPlaceholder: 'تکرار رمز عبور*',
      notEqualErrorLabel: 'رمز عبور مطابق نیست',
      passwordErrorLabel: 'رمز عبور مورد نیاز است',
      nameErrorLabel: 'نام مورد نیاز است',
      emailErrorLabel: 'ایمیل مورد نیاز است',
      emailNotValidLabel: 'فرمت ایمیل را درست وارد کنید.',
      name: '',
      email: '',
      password: '',
      repeatPassword: '',
