          ...
          <div v-if="isUserLoggedIn" class="level">
            <div class="level-item has-text-centered">
              <div>
                ...
                <p class="heading">شما هم اکنون وارد شده‌اید</p>
              </div>
            </div>
          </div>
        </section>
        <footer class="modal-card-foot">
          <button v-if="!isUserLoggedIn" type="submit" class="button is-info font17">{{ primaryBtnLabel }}</button>
          <button v-if="isUserLoggedIn" type="button" class="button is-info font17" @click="closeModal">{{ btnLoggedInLabel }}</button>
        </footer>
      </form>
    </div>
  </div>
...
export default {
  ...
  data () {
    return {
      modalTitle: 'ورود',
      modalTitleLoggedIn: 'خوش آمدید',
      primaryBtnLabel: 'ورود',
      emailRequiredLabel: 'ایمیل مورد نیاز است',
      passwordRequiredLabel: 'پسورد مورد نیاز است',
      emailNotValidLabel: 'ایمیل را به درستی وارد کنید',
      btnLoggedInLabel: 'بستن',
